...
class Encoder(nn.Module):
    ...
    def forward_by_stage(self, inputs,hidden_states_Prev, subnet, rnn):
        ...
        inputs = torch.reshape(inputs, (-1, input_channel, height, width))  #S*B,2,64,64
      
        
       
        inputs = subnet(inputs)   #subnets is made in subnet file
        inputs = torch.reshape(inputs, (seq_number, batch_size, inputs.size(1),   ##0 seq has 10 batch then 1st has 10 (15(S*B)=1,5(s,b))
    ...
    def forward(self, inputs,hidden_states_Prev):
        inputs = inputs.transpose(0, 1)  # to S,B,2,64,64
