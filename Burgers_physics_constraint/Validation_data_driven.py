...
'''you could run this file after dowloding the data file to obtain the results'''
...
TIMESTAMP = "short"
tsteps=19  #number of forward passes to be done through neural network
CKPT='checkpoint_146_0.007468.pth'
PL=True  # Plot Loss
########################################################################################
x_1 = np.linspace(0.1,1,95,endpoint=True)
random_seed = 199
...
TD_I = np.load('data_PC/dataset_input_PC.npy')  #1,19,5,2,64,64
TD_T = np.load('data_PC/dataset_input_PC_Valid_5_dt05.npy')

IS=0  # simulation to test
TD_I = torch.from_numpy(TD_I)
TD_T = torch.from_numpy(TD_T[IS,:19])

from pylab import*

load_dir='./burgers_PC/' + TIMESTAMP
...
if PL==True:
    ...
    f = open(os.path.join(load_dir)+'/avg_valid_loss.txt','r')
...
with torch.no_grad():
    ...
    loss_log=[]
    TD_I=TD_I[IS]
    Z = np.zeros((TD_T.shape[0], args.frames_input, 2, 64, 64))
    ...
    plt.plot(x_1,error_loss, color = 'b', label = 'MSE loss at each time step')
