...
class BurgerLoader():
    ...
    '''
    def __init__(self, dt=0.01, shuffle=False):
    ...
    def createTrainingLoader(self, ncases, nel, batch_size=64):
        ...
        dataSet = InitPeriodicCond2d(nel, ncases) #created periodic condition based on fourier series

        training_loader = DataLoader( dataSet,
            batch_size=batch_size, shuffle=self.shuffle, drop_last=True) #drops the smaller non equal batch size at last
        # Save original training loader
        self.training_loader0 = training_loader
        return training_loader

    def createTestingLoader(self, data_dir, cases, tMax=1.0, simdt=0.005, save_every=2, batch_size=1):
        '''
        ...
        # Create data loader
        data_tuple = (torch.cat(testing_data, dim=0), torch.stack(target_data, dim=0))  #0 is testing or initial data and target is 1 [2,101,2,64,64] where dim0 is no of cases
...
class InitPeriodicCond2d(torch.utils.data.Dataset):
    """Generate periodic initial condition on the fly.   #uNDERSTAND THE    formula?

    Args:
        order (int): order of Fourier series expansion #Need to check this
